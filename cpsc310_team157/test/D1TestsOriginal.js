...
describe("D1Tests", function () {
    ...
    it("check query should fail 424, rooms", function () {
        this.timeout(10000);
        return insF.performQuery({ "WHERE": { "OR": [{ "AND": [{ "GT": { "rooms_seats": 100 } }, { "OR": [{ "LT": { "rooms_lat": 40 } }, { "IS": { "rooms_name": "*AN*" } }, { "AND": [{ "IS": { "rooms_href": "*AN*" } }, { "EQ": { "rooms_seats": 54 } }] }] }, { "NOT": { "NOT": { "AND": [{ "LT": { "rooms_lon": 200 } }, { "GT": { "rooms_lon": -200 } }, { "NOT": { "IS": { "rooms_address": "*grono*" } } }] } } }] }, { "EQ": { "rooms_seats": 54 } }, { "NOT": { "IS": { "rooms_furniture": "Classroom-Movable Tables & Chairs" } } }] }, "OPTIONS": { "COLUMNS": ["rooms_name"], "ORDER": "rooms_name" } }).then(function (value) {
            Util_1.default.test('Value: ' + value);
            Util_1.default.test('Stringified Value: ' + value);
            Util_1.default.test('Length: ' + Object.keys(value.body).length);
            ...
            chai_1.expect(JSON.stringify(err)).to.equal('{"code":424,"body":{"error":"Valid database not found"}}');
        });
    });
    ...
    it("Attempt Empty WHERE: Expected Result: Success, Response Code: 200", function () {
        return insF.performQuery({
            ...
            chai_1.expect(resultFinal).to.deep.equal('{"result":[{"rooms_name":"DMP_101"},{"rooms_name":"DMP_110"},{"rooms_name":"DMP_201"},{"rooms_name":"DMP_301"},{"rooms_name":"DMP_310"}]}');
        }).catch(function (err) {
            Util_1.default.test('Error: ' + JSON.stringify(err));
            chai_1.expect.fail(err);
        });
    });
    it("check string query should fail 400, rooms", function () {
        this.timeout(10000);
        return insF.performQuery('{ "WHERE": { "OR":[{ "AND":[{ "GT":{ "rooms_seats":100 }},{"OR": [{"LT":{ "rooms_lat": 40 } },{ "IS":{ "rooms_name":"*AN*" } },{ "AND":[{ "IS":{ "rooms_href":"*AN*" } },{ "EQ":{ "rooms_seats": 54 } } ]}]},{ "NOT":{ "NOT":{ "AND":[{ "LT":{ "rooms_lon":200 } },{ "GT":{ "rooms_lon":-200 } },{ "NOT":{ "IS":{ "rooms_address": "*grono*" } } }] } } } ]},{"EQ":{ "rooms_seats":54 }},{ "NOT":{ "IS":{ "rooms_furniture":"Classroom-Movable Tables & Chairs" } } }] }, "OPTIONS":{ "COLUMNS":[ "rooms_name" ], "ORDER": "rooms_name" } }').then(function (value) {
            Util_1.default.test('Value: ' + value);
            Util_1.default.test('Stringified Value: ' + value);
    ...
    it("rooms.zip should return 201", function () {
        this.timeout(20000);
        return insF.addDataset("rooms", unP.getFromZip("rooms.zip").toString("base64")).then(function (value) {
    ...
    it("courses.zip should return 201", function () {
        this.timeout(20000);
        return insF.addDataset("courses", unP.getFromZip("courses.zip").toString("base64")).then(function (value) {
            chai_1.expect(value.code).to.equal(201);
            chai_1.expect(value.body).to.deep.equal({ "Message": "Data Updated!" });
    ...
    it("remove rooms.zip should return 204", function () {
        this.timeout(20000);
        insF.removeDataset("rooms").then(function (value) {
            chai_1.expect(value.code).to.equal(204);
            chai_1.expect(value.body).to.deep.equal({ "Message": "deleted" });
        }).catch(function (err) {
            Util_1.default.test('Error: ' + err);
            chai_1.expect.fail();
        });
    });
    it("remove courses.zip should return 204", function () {
        this.timeout(20000);
        insF.removeDataset("courses").then(function (value) {
            chai_1.expect(value.code).to.equal(204);
            chai_1.expect(value.body).to.deep.equal({ "Message": "deleted" });
        }).catch(function (err) {
            Util_1.default.test('Error: ' + err);
    ...
    it("remove courses.zip should return 404", function () {
        this.timeout(20000);
        insF.removeDataset("courses").then(function (value) {
        }).catch(function (err) {
            Util_1.default.test('Error: ' + err);
            chai_1.expect(err.code).to.equal(404);
            chai_1.expect(err.body).to.deep.equal({ "error": "Dataset does not exist in database!" });
        });
    });
    it("remove rooms.zip should return 404", function () {
        this.timeout(20000);
        insF.removeDataset("courses").then(function (value) {
        }).catch(function (err) {
            Util_1.default.test('Error: ' + err);
    ...
    it("courses.zip add 204, add 201, add 204, ARSENIC", function () {
        this.timeout(20000);
        return insF.addDataset("arsenic", unP.getFromZip("courses2.zip").toString("base64")).then(function (value) {
            Util_1.default.test('Code 204 Expected: ' + value.code);
            return insF.addDataset("arsenic", unP.getFromZip("courses2.zip").toString("base64")).then(function (value) {
                Util_1.default.test('Code 201 Expected: ' + value.code);
                return insF.addDataset("rooms", unP.getFromZip("rooms.zip").toString("base64")).then(function (value) {
                    Util_1.default.test('Code 204 Expected:' + value.code);
                    chai_1.expect(value.code).to.equal(204);
                    chai_1.expect(value.body).to.deep.equal({ "Message": "Data Added!" });
    ...
    it("aresenic/cobalt", function () {
        this.timeout(20000);
        return insF.addDataset("courses.zip", unP.getFromZip("courses.zip").toString("base64")).then(function (value) {
            var insF2 = new InsightFacade_1.default();
            insF2.addDataset("rooms.zip", unP.getFromZip("rooms.zip").toString("base64")).then(function (valuex) {
                Util_1.default.test('Value: ' + valuex);
                Util_1.default.test('Stringified Value: ' + JSON.stringify(valuex));
                chai_1.expect(value.code).to.equal(204);
                chai_1.expect(value.body).to.deep.equal({ "Message": "Data Added!" });
            }).catch(function (err) {
                Util_1.default.test('Error: ' + err);
    ...
    it("courses.zip should return 204, ARSENIC", function () {
        this.timeout(20000);
        return insF.addDataset("courses", unP.getFromZip("courses.zip").toString("base64")).then(function (value) {
            chai_1.expect(value.code).to.equal(204);
            chai_1.expect(value.body).to.deep.equal({ "Message": "Data Added!" });
            ...
            var insF2;
            var unP2;
            insF2 = new InsightFacade_1.default();
            unP2 = new Unpack_1.Unpack();
            insF2.addDataset("rooms", unP2.getFromZip("rooms.zip").toString("base64")).then(function (value) {
                chai_1.expect(value.code).to.equal(204);
                chai_1.expect(value.body).to.deep.equal({ "Message": "Data Added!" });
            }).catch(function (err) {
                Util_1.default.test('Error: ' + err);
    ...
    it("rooms.zip should return 201, ARSENIC", function () {
        this.timeout(20000);
        return insF.addDataset("rooms", unP.getFromZip("rooms.zip").toString("base64")).then(function (value) {
            chai_1.expect(value.code).to.equal(201);
            chai_1.expect(value.body).to.deep.equal({ "Message": "Data Updated!" });
            ...
            insF2 = new InsightFacade_1.default();
            unP2 = new Unpack_1.Unpack();
            insF2.addDataset("courses", unP2.getFromZip("courses.zip").toString("base64")).then(function (value) {
                chai_1.expect(value.code).to.equal(201);
                chai_1.expect(value.body).to.deep.equal({ "Message": "Data Updated!" });
            }).catch(function (err) {
                Util_1.default.test('Error: ' + err);
...
//# sourceMappingURL=D1TestsOriginal.js.map