...
describe("D1Tests", function () {
    ...
    it("check string query should fail 400, rooms", function () {
        ...
        //return insF.addDataset("courses",unP.getFromZip("courses.zip").toString("base64"))
        //var query = fs.readFileSync('query3.json');
        //query = JSON.parse(query);
        return insF.performQuery('{ "WHERE": { "OR":[{ "AND":[{ "GT":{ "rooms_seats":100 }},{"OR": [{"LT":{ "rooms_lat": 40 } },{ "IS":{ "rooms_name":"*AN*" } },{ "AND":[{ "IS":{ "rooms_href":"*AN*" } },{ "EQ":{ "rooms_seats": 54 } } ]}]},{ "NOT":{ "NOT":{ "AND":[{ "LT":{ "rooms_lon":200 } },{ "GT":{ "rooms_lon":-200 } },{ "NOT":{ "IS":{ "rooms_address": "*grono*" } } }] } } } ]},{"EQ":{ "rooms_seats":54 }},{ "NOT":{ "IS":{ "rooms_furniture":"Classroom-Movable Tables & Chairs" } } }] }, "OPTIONS":{ "COLUMNS":[ "rooms_name" ], "ORDER": "rooms_name" } }').then(function (value: InsightResponse) {
            Log.test('Value: ' + value);
    ...
    it("courses.zip add 204, add 201, add 204, ARSENIC", function () {
        this.timeout(20000);
        return insF.addDataset("arsenic", unP.getFromZip("courses2.zip").toString("base64")).then(function (value: InsightResponse){
            Log.test('Code 204 Expected: ' + value.code);
            return insF.addDataset("arsenic", unP.getFromZip("courses2.zip").toString("base64")).then(function (value: InsightResponse) {
                Log.test('Code 201 Expected: ' + value.code);
                return insF.addDataset("rooms", unP.getFromZip("rooms.zip").toString("base64")).then(function (value: InsightResponse) {
                    Log.test('Code 204 Expected:' + value.code);
                    expect(value.code).to.equal(204);
                    expect(value.body).to.deep.equal({"Message": "Data Added!"});
    ...
    it("aresenic/cobalt", function () {
        this.timeout(20000);
        return insF.addDataset("courses.zip", unP.getFromZip("courses.zip").toString("base64")).then(function (value: InsightResponse){
            //Log.test('Value: ' + value);
            //Log.test('Stringified Value: ' + JSON.stringify(value));
            var insF2 = new InsightFacade();
            insF2.addDataset("rooms.zip", unP.getFromZip("rooms.zip").toString("base64")).then(function (valuex: InsightResponse){
    ...
    //does not add both datasets to database as reflected by next test
    it("courses.zip should return 204, ARSENIC", function () {
        ...
        return insF.addDataset("courses", unP.getFromZip("courses.zip").toString("base64")).then(function (value: InsightResponse) {
            ...
            unP2 = new Unpack();
            insF2.addDataset("rooms", unP2.getFromZip("rooms.zip").toString("base64")).then(function (value: InsightResponse) {
                //Log.test('Value: ' + value);
                //Log.test('Stringified Value: ' + JSON.stringify(value));
                expect(value.code).to.equal(204);
                expect(value.body).to.deep.equal({"Message": "Data Added!"});
    ...
    it("rooms.zip should return 201, ARSENIC", function () {
        ...
        return insF.addDataset("rooms", unP.getFromZip("rooms.zip").toString("base64")).then(function (value: InsightResponse) {
            ...
            insF2 = new InsightFacade();
            unP2 = new Unpack();
            insF2.addDataset("courses", unP2.getFromZip("courses.zip").toString("base64")).then(function (value: InsightResponse) {
                //Log.test('Value: ' + value);
                //Log.test('Stringified Value: ' + JSON.stringify(value));
                expect(value.code).to.equal(201);
                expect(value.body).to.deep.equal({"Message": "Data Updated!"});
